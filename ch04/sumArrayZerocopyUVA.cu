...
{
    std::cout << "sumArrayZerocpyUVA program starts ..." << std::endl;
    ...
    std::cout << "sumArrays: " << GetDurationInMicroSeconds(begin, StopTimer()) << " microsecs" << std::endl;
    ...
    CHECK(cudaDeviceSynchronize());
    std::cout << "sumArraysZeroCopy: " << GetDurationInMicroSeconds(begin, StopTimer()) << " microsecs" << std::endl;
    // copy kernel result back to host side
    CHECK(cudaMemcpy(gpuRef, d_C, nBytes, cudaMemcpyDeviceToHost));
    ...
    CHECK(cudaDeviceSynchronize());
    std::cout << "sumArraysCopyWithUVA: " << GetDurationInMicroSeconds(begin, StopTimer()) << " microsecs" << std::endl;
    // copy kernel result back to host side
    CHECK(cudaMemcpy(gpuRef, d_C, nBytes, cudaMemcpyDeviceToHost));
