...
// one thread handles i-th element's & (i+nthreads)-th element's addition
__global__ void sumArraysOnGPU(float *A, float *B, float *C, const int N) {
    // Cacluate total # of threads launched across all blocks
    int nthreads = gridDim.x * blockDim.x;
    // Calculate the global thread index
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    int j = i + nthreads; // jump nthreads ahead
...
int main(int argc, char **argv) {
    ...
    int iLen = 256; // Exerice 2
    dim3 block (iLen); // 256 threads per block - only one dimension .x
    dim3 grid (((nElem / 2) + block.x - 1)/block.x); // Exerice 2 - half as many threads needed
