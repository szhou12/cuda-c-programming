...
__global__ void sumMatrixOnGPU(float *MatA, float *MatB, float *MatC, const int nx, const int ny)
{
    ...
    unsigned int jx = ix + nxthreads;

    unsigned int idx = iy * nx + ix;
    unsigned int jdx = iy * nx + jx;

    if (iy < ny)
    {
        if (ix < nx) MatC[idx] = MatA[idx] + MatB[idx];
        if (jx < nx) MatC[jdx] = MatA[jdx] + MatB[jdx];
    }
}
...
int main(int argc, char **argv)
{
    ...
    dim3 block(dimx, 1);
    dim3 grid(((nx / 2) + block.x - 1) / block.x, ny); // cut total threads in half
