...
#define MAD(x,y,z)  __fmaf_rn(x,y,z)
#define RECP(x)     __frcp_rn(x)
...
#define MAD(x,y,z)  __fma_rn(x,y,z)
#define RECP(x)     __drcp_rn(x)
                                        ...
                                        int N, int *exceeded_speed)
{
    ...
    do
    {
        ...
        real dist2 = MAD(rx, rx,MUL(ry, ry));
        ...
        real s = MUL(MASS, RECP(SQRT(dist6)));
        total_ax = MAD(rx, s, total_ax);
        total_ay = MAD(ry, s, total_ay);
    ...
    real v = SQRT(MAD(vx[tid], vx[tid], MUL(vy[tid], vy[tid])));
                                        ...
                                        int N, int *beyond_bounds)
{
    ...
    px[tid] = MAD(vx[tid], DT, px[tid]);
    py[tid] = MAD(vy[tid], DT, py[tid]);

    real dist = SQRT(POW(px[tid], TWO) + POW(py[tid], TWO));
...
int main(int argc, char **argv)
{
    ...
    printf("Any points beyond bounds? %s, # points exceeded velocity %d/%d\n",
           niters, beyond_bounds > 0 ? "true" : "false", exceeded_speed,
           N);
