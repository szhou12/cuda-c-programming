                                        ...
                                        int N, int *exceeded_speed)
{
    ...
    // Perform parallel, intra-block reduction
    if (v > MAX_SPEED)
    {
        smem[threadIdx.x] = 1;
    } else {
    ...
    __syncthreads();
    if (blockDim.x >= 64 && threadIdx.x < 32) smem[threadIdx.x] += smem[threadIdx.x + 32];
    __syncthreads();

    int localSum = smem[threadIdx.x];
    ...
    // Add each thread block's result to the global sum
    if (threadIdx.x == 0) {
        atomicAdd(exceeded_speed, localSum);
    }
}
