...
/**
* Implements a recursive CPU-based reduction algorithm
* Uses the interleaved pair approach where elements are paired from opposite ends
* Recursively reduces the array size by half until only one element remains
...
/**
 * Basic neighboring pair reduction with high thread divergence
 * Each thread adds its neighbor based on stride pattern (1, 2, 4, 8, ...)
 * Uses modulo operation (tid % (2 * stride)) == 0 which causes branch divergence
 * Less efficient due to warp divergence
 */
...
/**
 * Improved neighboring pair reduction with reduced divergence
 * Uses index-based approach instead of modulo to reduce branch divergence
 * Calculates index = 2 * stride * tid to determine which threads participate
 * Better performance than the basic neighboring approach
 */
...
/**
 * Implements interleaved pair reduction approach
 * Starts with large stride (blockDim.x / 2) and reduces by half each iteration
 * More efficient memory access pattern and better thread utilization
 * Reduces thread divergence compared to neighboring approaches
 */
...
/**
 * reduceUnrolling2/4/8/16
 * Implements loop unrolling optimization with factors of 2, 4, 8, and 16
 * Each thread processes multiple elements before the main reduction loop
 * Reduces the number of blocks needed by the unrolling factor
 * Higher unrolling factors process more elements per thread initially
 */
...
/** 
 * Combines 8x loop unrolling with warp-level optimization
 * Unrolls the final warp iterations (last 32 threads) manually
 * Uses volatile keyword to prevent compiler optimizations that could break the warp-level code
 * Eliminates __syncthreads() calls within the final warp since threads in a warp execute in lockstep
 */
...
/** 
 * Further optimization that completely unrolls the reduction loop
 * Manually unrolls specific block sizes (1024, 512, 256, 128, 64)
 * Combines complete loop unrolling with warp-level optimizations
 * Most optimized version for the 8x unrolling case
 */
...
/**
 * Template-based version that uses compile-time constants for block sizes
 * Allows compiler to optimize away conditional branches since block size is known at compile time
 * Uses template parameter iBlockSize for maximum optimization
 * Most flexible and optimized approach
 */
...
/**
 * Simpler version with 2x unrolling and warp optimization
 * Good balance between complexity and performance
 * Uses the same warp-level unrolling technique as the 8x version
 */
